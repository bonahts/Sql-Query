DROP TABLE IF EXISTS CLM.CESA_C0B016_BASE_01 ;
CREATE TABLE CLM.CESA_C0B016_BASE_01 WITH (APPENDONLY=TRUE, COMPRESSLEVEL=5, ORIENTATION=COLUMN) AS
(	
	SELECT DISTINCT SUBS_ID
		, TRX_DATE::DATE
		, DEST_DESC
		, VOL_UNL_FLAG
		, ROW_NUMBER() OVER (PARTITION BY SUBS_ID ORDER BY TRX_DATE DESC) AS RN 
	FROM
        DWH_CC.F_NON_TRAFFIC A 
    WHERE
...
DROP TABLE IF EXISTS CLM.CESA_C0B016_BASE_02 ;
CREATE TABLE CLM.CESA_C0B016_BASE_02 WITH (APPENDONLY=TRUE, COMPRESSLEVEL=5, ORIENTATION=COLUMN) AS
(	
	SELECT DISTINCT SUBS_ID
		, TRX_DATE::DATE
		, DEST_DESC
		, CASE 
			WHEN UPPER(DEST_DESC) LIKE '%NON%STOP%' THEN 'NON STOP'
			WHEN UPPER(VOL_UNL_FLAG) = 'UNLIMITED' THEN 'UNLIMITED'
			WHEN UPPER(VOL_UNL_FLAG) = 'VOLUME' THEN 'VOLUME'
			ELSE 'OTHER'
			END AS PACKAGE_CATEGORY
		, VOL_UNL_FLAG	
	FROM
        CLM.CESA_C0B016_BASE_01 A 
    WHERE
		RN = 1
)
DISTRIBUTED BY (SUBS_ID)
...
DROP TABLE IF EXISTS CLM.CESA_C0B016_BASE_03 ;
CREATE TABLE CLM.CESA_C0B016_BASE_03 WITH (APPENDONLY=TRUE, COMPRESSLEVEL=2, ORIENTATION=COLUMN) AS
(
SELECT DISTINCT A.SUBS_ID
	, A.VOL_UNL_FLAG
	, A.PACKAGE_CATEGORY
	, CASE WHEN UPPER(PAGGR_STORE_CODE) = 'MYSF' THEN 'MYSF'
		ELSE 'NON MYSF'
		END AS PURCHASE_CHANNEL
	, A.DEST_DESC
FROM CLM.CESA_C0B016_BASE_02 A LEFT JOIN (	
	SELECT SUBS_ID, 
	PAGGR_STORE_CODE
	, PARTITION_DATE
	FROM RPT_CC.RPT_REVENUE_BY_CHANNEL
	WHERE UPPER(USAGE_TYPE) = 'EVENT CHARGE'
	AND TOTAL_USAGE > 0
	AND PARTITION_DATE BETWEEN CURRENT_DATE::DATE -31 AND CURRENT_DATE::DATE-1
	)B ON A.SUBS_ID =B.SUBS_ID AND A.TRX_DATE::DATE = B.PARTITION_DATE::DATE
)
DISTRIBUTED BY (SUBS_ID);


DROP TABLE IF EXISTS CLM.CESA_C0B016_STG;
CREATE TABLE CLM.CESA_C0B016_STG WITH (APPENDONLY=TRUE, COMPRESSLEVEL=5, ORIENTATION=COLUMN) AS
(
...
DROP TABLE IF EXISTS CLM.TEMP_MCCM_SEGMENT_C0B016_TARGET;
CREATE TABLE CLM.TEMP_MCCM_SEGMENT_C0B016_TARGET AS
(
SELECT
	 DISTINCT
	 SUBS_ID
...
from CLM.TEMP_MCCM_SEGMENT_C0B016_TARGET A
